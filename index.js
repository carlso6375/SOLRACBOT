...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'CALMA BAFO DE P�NIS TOU FAZENDO�',
				success: 'PRONTO FILHO DA PUTA�',
				...
				only: {
					group: '[❗] ESSA PORRA SO PODE SER USADO EM GRUPOS! ❌',
					ownerG: '[❗] ESSA PORRA SO PODE SER USADO GRUPO PROPRIET�RIO!(ninem/SOLRAC O MAIS LINDO ��) ❌',
					ownerB: '[❗] ESTE COMANDO SO PODE SER USADO PELO SOLARC LINDO /ninem! ❌',
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedido SAIA DO GRUPO SEU COMEDIA SOLRAC AQ :\n`
							teks += `@_.split('@')[0]`
						}
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('ONDE EST� (O LINK FILHO DA PUTA?'')
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					...
					break
				case 'pokemon':
                    client.updatePresence(CALMA AI SEU MERDA!!) 
				...
				case 'meme':
					...
					break
					case ' OI SOLRAC ':
		if (args.length < 1) return reply ('OI FILHO DA PUTA')
		break
				...
				case 'bc':
					...
					break
					case 'bcgc':
					client.updatePresence('ESPERA UM POUCO SEU GAY'') 
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('.......')
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						...
						reply('TA FEITO SEU FILHO DA PUTA')
					}
					break
				case 'alay':
                    client.updatePresence('ESPERA AI')
                    if (!isUser) return reply(mess.only.daftarB)
				...
				case 'chord':
					if (args.length < 1) return reply('ONDE ESTA O T�TULO LIXO?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(ESPERA AI COM�DIA ')
                                    if (!isUser) return reply(mess.only.daftarB)
					...
					break
                	case 'tomp3':
                	client.updatePresence(TOU FAZENDO ESSA MERDA') 
                        if (!isUser) return reply(mess.only.daftarB)
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('JA TA REGISTRADO CABE�ADE P�NIS')
